...
enum DisplayRestorationTests {
    ...
    class Fixture {
        ...
        var refreshes = 0
        // This fork's toggles carry a lifecycle generation and an identity
        // check; both are inputs here, not transports, so the restoration
        // paths below are the only thing under test.
        var running = true
        var lifecycleGeneration = 0
        var toggleTargetMatches = true
        func matchesToggleTarget(_ display: BrightnessDisplay) -> Bool { toggleTargetMatches }
        static func currentTopology() -> BrightnessSupport.DisplayTopology {
            BrightnessSupport.DisplayTopology(online: [1, 2], active: [1, 2])
        }
        static func drawableDisplayIDs(online: Set<UInt32>, active: Set<UInt32>) -> Set<UInt32> { active }
    ...
    static func run(_ suite: TestSuite) {
        ...
        DispatchQueue.main.async { [weak service] in
            service?.commitDisplayToggle(BrightnessDisplay(id: 1), enabled: true, generation: 0)
        }
        Hardware.lid = true
        DispatchQueue.main.drain()
        ...
        service.restoreDisplaysLeftOff()
        Hardware.lid = false
        service.commitDisplayToggle(BrightnessDisplay(id: 1, isActive: true), enabled: false, generation: 0)
        DispatchQueue.main.drain()
        ...
        // This fork refuses a toggle whose target no longer matches the display
        // it was raised for, and the restore-all path must honour that too.
        service = make()
        service.managedDisabledIDs = [1]
        service.managedDisabledDisplays[1] = BrightnessDisplay(id: 1)
        service.toggleTargetMatches = false
        Hardware.lid = false
        service.restoreManagedDisplays()
        service.commitDisplayToggle(BrightnessDisplay(id: 1), enabled: true, generation: 0)
        DispatchQueue.main.drain()
        suite.expect(Hardware.transactions == 0 && service.managedDisabledIDs == [1]
                     && service.displayControlFailure == .unavailable,
                     "a display that no longer matches its snapshot is never reconfigured")
        service.toggleTargetMatches = true
        service.displayControlFailure = nil
        service.lifecycleGeneration = 1
        service.commitDisplayToggle(BrightnessDisplay(id: 1), enabled: true, generation: 0)
        DispatchQueue.main.drain()
        suite.expect(Hardware.transactions == 0 && service.displayControlFailure == nil,
                     "a toggle from a previous feature lifecycle neither reconfigures nor reports")
    }
}
