...
enum ShelfFilePromiseTests {
    ...
    static func run(expect: @escaping (Bool, String) -> Void) {
        ...
        do {
            ...
            expect(directoryResult!.failed && directoryResult!.urls.isEmpty && entries(directoryStore).isEmpty,
                   "promised directories are rejected without retaining an unmanaged copy")
