        ...
        do {
            ...
            for category in [CleanerSupport.Category.caches, .logs] {
                ...
                appendLeftovers(in: dir.path, usesContainerMetadata: false,
                                installed: [], fm: manager, into: &leftovers, isCancelled: { false })
                let claimed = Set(leftovers.map { $0.url.standardizedFileURL.path })
                let found = category == .caches ? scanCaches(excluding: claimed, isCancelled: { false }) : scanLogs(excluding: claimed, isCancelled: { false })
                suite.expect(found.isEmpty,
                             "localized folders cannot return as recommended \(category) after leftover scanning")
                let normal = category == .caches ? scanCaches(excluding: [], isCancelled: { false }) : scanLogs(excluding: [], isCancelled: { false })
