...
enum UninstallerFlowTests {
    ...
    class UninstallerState {
        ...
        typealias HomebrewPackage = String
        ...
        var selectionError: UninstallerSelectionError?
    ...
    class ServiceState {
        ...
        var pendingHomebrewRemoval: AppUninstaller.HomebrewRemovalConfirmation?
        var activeCategory: String?
        var isPeekingHome = false
        var cachedApps: [InstalledApps.InstalledApp] = []
        var argumentSubmission = CommandBarArgumentSubmission()
        func invalidateDestinationAttempt() {}
    ...
    static func run(_ suite: TestSuite) {
        ...
        do {
            ...
            service.beginUninstallReview(appURL: b, entryID: "b")
            Queue.drain()
            Brew.shared.callback?("second")
            service.confirmUninstallReview(entryID: "b")
            suite.expect(service.mode == .uninstallHomebrewConfirm(entryID: "b")
                         && uninstaller.phase == .results && uninstaller.confirmed.isEmpty,
                         "a managed app asks for its own confirmation before anything is removed")
            let reviewed = Set(uninstaller.items.filter(\.include).map(\.id))
            service.confirmUninstallHomebrewRemoval(entryID: "b")
            suite.expect(uninstaller.confirmed.count == 1
                         && uninstaller.confirmed.first?.package == "second"
                         && uninstaller.confirmed.first?.targetURL == b
                         && uninstaller.confirmed.first?.selectedIDs == reviewed
                         && service.mode == .uninstallReview(entryID: "b"),
                         "the confirmation covers the reviewed package, app and items, then returns to the review")
            service.confirmUninstallHomebrewRemoval(entryID: "b")
            suite.expect(uninstaller.confirmed.count == 1, "a confirmation that was used cannot be replayed")
            service.stepBack()
            service.beginUninstallReview(appURL: a, entryID: "a")
            Queue.drain()
            Brew.shared.callback?(nil)
            service.confirmUninstallReview(entryID: "a")
            suite.expect(uninstaller.phase == .removing && uninstaller.confirmed.count == 1
                         && service.mode == .uninstallReview(entryID: "a"),
                         "an unmanaged app moves to the Trash from the review without a second step")
