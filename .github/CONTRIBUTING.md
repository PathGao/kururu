...
The command bar learns which durable results you choose to improve local ranking. Preferences retain result identifiers, use counts and last-use times. Which result won for a typed query is kept only in the running process, as keyed digests of query prefixes under a random key that never leaves memory; the original query text is not saved, and no Keychain item is used. Query text and short-term matching memory are held in the running process. You can forget a result's ranking from its action panel or clear learned ranking in command bar settings. These ranking records are not sent to a server.
