...
struct WindowPreviewExclusionsList: View {
    ...
    init(key: String) {
        self.key = key
        _apps = State(initialValue: Self.savedApps(key))
    }
    ...
    private static func savedApps(_ key: String) -> [String] {
        Defaults.sanitizedBundleIdentifierList(UserDefaults.standard.stringArray(forKey: key) ?? [])
    }
