...
struct WindowLayoutSettings: View {
    ...
    var body: some View {
        SettingsForm {
            ...
            SettingsSection(text.gestureSection) {
            ...
            SettingsSection(text.gapsSection) {
            ...
            SettingsSection {
                Toggle(FeatureStrings.dockClick(l10n.language).cycleWindows, isOn: $dockClickCycleWindows)
                    .onChange(of: dockClickCycleWindows) { _, _ in
                        DockClickService.shared.syncWithPreferences()
                    }
                Text(FeatureStrings.dockClick(l10n.language).cycleWindowsCaption)
                    .font(SettingsTypography.caption)
                    .foregroundStyle(.secondary)
            }

            SettingsSection(text.halves) {
                actionRow(.leftHalf)
                actionRow(.rightHalf)
                actionRow(.topHalf)
...
/// Shared by this page and the central shortcuts page, which shows the same
/// rows without the try-it button.
struct WindowLayoutActionRow: View {
    @ObservedObject private var l10n = L10n.shared
