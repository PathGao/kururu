...
struct CutPasteSettings: View {
    ...
    var body: some View {
        Form {
            FeatureSwitchSection(unit: .cutPaste)
            if AppFeature.finderCutPaste.isAvailable {
                Section {
                    ...
                    Toggle(FeatureStrings.clipboard(l10n.language).pasteImageAsFile, isOn: $pasteImageAsFile)
                        .onChange(of: pasteImageAsFile) { _, _ in
                            FinderCutPaste.shared.syncWithPreferences()
                        }
                    Text(FeatureStrings.clipboard(l10n.language).pasteImageAsFileCaption)
                        .font(.caption)
                        .foregroundStyle(.secondary)
            ...
            if AppFeature.finderRename.isAvailable {
                Section {
                    ...
                    Text(AppFeature.finderRename.name(l10n.s, language: l10n.language))
                }
