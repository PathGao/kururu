...
extension Strings {
    static let ru = Strings(
        statusIdleTooltip: "\(AppInfo.name): обычный сон",
        statusActiveUntil: "\(AppInfo.name): активно до",
        statusActiveIndefinite: "\(AppInfo.name): активно без срока",
        menuEnableAwake: "Включить режим без сна",
        menuDisableAwake: "Выключить режим без сна",
        menuActivateFor: "Включить на…",
        menuSettings: "Настройки…",
        menuAbout: "О \(AppInfo.name)",
        menuQuit: "Выйти из \(AppInfo.name)",
        menuHide: "Скрыть \(AppInfo.name)",
        ...
        notifyBatteryTitle: "\(AppInfo.name) отключён",
        notifyBatteryBody: "Низкий заряд. Обычный сон восстановлен, чтобы сохранить батарею.",
        adminPromptClamshellOff: "\(AppInfo.name) нужен ваш пароль, чтобы вернуть обычный режим сна Mac.",
        adminPromptRecover: "\(AppInfo.name) завершился, пока сон Mac был отключён. Введите пароль, чтобы вернуть обычный режим сна.",
        adminPromptUpdate: "\(AppInfo.name) нужен ваш пароль, чтобы установить обновление.",
        adminPromptSudoersInstall: "\(AppInfo.name) создаст ограниченное правило (только pmset disablesleep), чтобы переключать режим закрытой крышки без запроса пароля. Пароль понадобится только сейчас.",
        adminPromptSudoersRemove: "\(AppInfo.name) удалит правило для режима закрытой крышки без пароля.",

        settingsTitle: "Настройки \(AppInfo.name)",
        tabGeneral: "Основное",
        tabMouse: "Мышь",
        tabDock: "Dock",
        tabWindowBehavior: "Поведение окон",
        ...
        advancedResetDescription: "Удаляет все разрешения, которые вы дали \(AppInfo.name) (Универсальный доступ, Запись экрана, Полный доступ к диску и другие), элемент входа и правило закрытой крышки. Полезно, если хотите начать с чистого листа или перед удалением. Приложение останется установленным.",
        ...
        advancedUninstallButton: "Удалить \(AppInfo.name) полностью",
        advancedUninstallConfirmTitle: "Удалить \(AppInfo.name)?",
        advancedUninstallConfirmBody: "\(AppInfo.name) очистит разрешения, удалит настройки и переместится в Корзину, после чего закроется. Это нельзя отменить из приложения, но файл останется в Корзине, пока вы её не очистите.",
        advancedUninstallFailedTitle: "Удаление остановлено",
        advancedUninstallFailedBody: "\(AppInfo.name) не смог вернуть изменённую системную настройку: режим сна, скорость вентиляторов или ускорение мыши. Ничего не удалено. Повторите попытку и разрешите запрос пароля, если он появится.",
        ...
        keepAwakeAutoStart: "Не давать уснуть при запуске \(AppInfo.name)",
        ...
        switcherEnable: "Использовать переключатель \(AppInfo.name)",
        ...
        switcherTakeOverSystemShortcutsCaption: "Отключает соответствующие сочетания macOS только на время работы переключателя \(AppInfo.name). Все запущенные приложения остаются доступны.",
        ...
        uninstallerFDAHint: "Включите \(AppInfo.name) в списке. Если его там нет, нажмите + и выберите \(AppInfo.name) из папки «Программы». Доступ начнёт действовать только после повторного запуска приложения.",
        ...
        uninstallerFreedFormat: "Удалено данных: %@",
        ...
        homebrewTerminalFallback: "Для этой операции нужен Терминал, чтобы запросить пароль администратора. \(AppInfo.name) не перехватывает пароли.",
        ...
        updateNotifyTitle: "Обновление \(AppInfo.name)",
        ...
        updateNeedsApplicationsTitle: "Переместите \(AppInfo.name) в папку «Программы»",
        updateNeedsApplicationsBody: "Приложение запущено из места, которое нельзя обновить, например из образа диска или временного системного расположения. Перетащите \(AppInfo.name) в папку «Программы», откройте его оттуда и попробуйте снова.",
        ...
        obContinue: "Далее",
        obBack: "Назад",
        obStart: "Открыть \(AppInfo.name)",
        obStepWelcomeTitle: "Добро пожаловать в \(AppInfo.name)",
        ...
        obStepDoneBody: "\(AppInfo.name) уже заботится о вашем Mac.",
        ...
        obPurposeBody: "Выберите отдельные функции для включения. Текущие настройки сохранятся.",
        obPurposeSkip: "Функции можно добавить или убрать позже в настройках.",

        tabMonitor: "Монитор",
        tabMenuBarIcon: "Значок в строке меню",
        tabMenuBarPanel: "Панель строки меню",
        ...
        menuBarIconSection: "Значок приложения",
        ...
        cleaningNeedsAxBody: "Чтобы безопасно блокировать клавиатуру, \(AppInfo.name) нужен доступ к Универсальному доступу. Выдайте его в Системных настройках и попробуйте снова.",
        ...
        showMenuBarIconCaption: "Если значок \(AppInfo.name) исчезнет (macOS может скрывать значки строки меню, когда в ней не хватает места, что часто бывает на Mac с вырезом), откройте \(AppInfo.name) снова из папки «Программы» или через Spotlight: это пересоздаст значок и, если он всё ещё скрыт, откроет это окно.",
        ...
        menuBarIconManagerHintFormat: "%@ запущен и, возможно, держит значок в своей скрытой секции. Поищите \(AppInfo.name) там или настройте %@ всегда показывать \(AppInfo.name).",
        ...
        cleanerNotifDenied: "Уведомления \(AppInfo.name) отключены в системе.",
        cleanerNotifOpenSettings: "Открыть настройки уведомлений…",
        launchAtLoginNeedsApplications: "Приложение запущено из места, из которого нельзя открываться при входе. Перетащите \(AppInfo.name) в папку «Программы», откройте его оттуда и включите параметр снова.",
        launchAtLoginNeedsApproval: "Объект входа зарегистрирован, но в Системных настройках он выключен. Откройте «Системные настройки › Основные › Объекты входа и расширения» и включите \(AppInfo.name) в разделе «Открывать при входе».",
        ...
        mouseAccelerationCaption: "Отключает ускорение указателя для подключённых мышей. Прежняя настройка вернётся после выключения функции или выхода из \(AppInfo.name).",
