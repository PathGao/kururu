...
extension Strings {
    static let tr = Strings(
        statusIdleTooltip: "\(AppInfo.name): normal uyku",
        statusActiveUntil: "\(AppInfo.name): şu zamana kadar uyanık",
        statusActiveIndefinite: "\(AppInfo.name): süresiz uyanık",
        ...
        menuAbout: "\(AppInfo.name) Hakkında",
        menuQuit: "\(AppInfo.name)’ten Çık",
        menuHide: "\(AppInfo.name)’i Gizle",
        ...
        notifyBatteryTitle: "\(AppInfo.name) devre dışı",
        notifyBatteryBody: "Pil düşük. Şarjı korumak için normal uyku geri yüklendi.",
        adminPromptClamshellOff: "\(AppInfo.name)’in, Mac’in normal uykusunu geri yüklemek için parolana ihtiyacı var.",
        adminPromptRecover: "Mac’in uykusu kapalıyken \(AppInfo.name) kapandı. Normal uykuyu geri yüklemek için parolayı gir.",
        adminPromptUpdate: "\(AppInfo.name) güncellemeyi yüklemek için parolanıza ihtiyaç duyuyor.",
        adminPromptSudoersInstall: "\(AppInfo.name), parola sormadan kapak kapalı modunu açıp kapatmak için kısıtlı bir kural oluşturacak (yalnızca pmset disablesleep). Parolanın gerektiği tek zaman bu.",
        adminPromptSudoersRemove: "\(AppInfo.name), parolasız kapak kapalı kuralını kaldıracak.",

        settingsTitle: "\(AppInfo.name) Ayarları",
        tabGeneral: "Genel",
        tabMouse: "Fare",
        tabDock: "Dock",
        tabWindowBehavior: "Pencere davranışı",
        tabKeyboard: "Klavye",
        tabTrackpad: "İzleme dörtgeni",
        tabAdvanced: "Gelişmiş",
        tabAbout: "Hakkında",
        tabReleaseNotes: "Yenilikler",
        advancedResetSection: "İzinler",
        advancedResetDescription: "\(AppInfo.name)’e verdiğin tüm izinleri (Erişilebilirlik, Ekran Kaydı, Tam Disk Erişimi ve diğerleri), oturum açma öğesini ve kapak kapalı kuralını kaldırır. Sıfırdan başlamak veya kaldırmadan önce temizlemek için kullanışlıdır. Uygulama yüklü kalır.",
        ...
        advancedUninstallButton: "\(AppInfo.name)’i tamamen kaldır",
        advancedUninstallConfirmTitle: "\(AppInfo.name) kaldırılsın mı?",
        advancedUninstallConfirmBody: "\(AppInfo.name) izinlerini temizleyecek, tercihlerini kaldıracak ve Çöp Sepeti’ne taşınıp kapanacak. Bu işlem uygulama içinden geri alınamaz, ama Çöp Sepeti’ni boşaltana kadar orada kalır.",
        advancedUninstallFailedTitle: "Kaldırma durduruldu",
        advancedUninstallFailedBody: "\(AppInfo.name) değiştirdiği bir sistem ayarını geri alamadı: uyku, fan hızı veya fare hızlandırması. Hiçbir şey silinmedi. Yeniden deneyin ve parola istenirse izin verin.",
        ...
        keepAwakeAutoStart: "\(AppInfo.name) açılınca uyanık tut",
        ...
        switcherEnable: "\(AppInfo.name) değiştiricisini kullan",
        ...
        switcherTakeOverSystemShortcutsCaption: "Eşleşen macOS uygulama ve pencere kısayollarını yalnızca \(AppInfo.name) değiştiricisi etkinken kapatır. Çalışan tüm uygulamalara erişim korunur.",
        ...
        uninstallerFDAHint: "Listede \(AppInfo.name)’i aç. Orada yoksa + düğmesine tıkla ve Uygulamalar’dan \(AppInfo.name)’i seç. Erişim yalnızca uygulamayı yeniden açtıktan sonra geçerli olur.",
        ...
        uninstallerFreedFormat: "Kaldırılan veri: %@",
        ...
        homebrewTerminalFallback: "Bu işlem, yönetici parolasını istemek için Terminal’e ihtiyaç duyar. \(AppInfo.name) parolaları yakalamaz.",
        ...
        updateNotifyTitle: "\(AppInfo.name) güncellemesi",
        ...
        updateNeedsApplicationsTitle: "\(AppInfo.name)’i Uygulamalar klasörüne taşıyın",
        updateNeedsApplicationsBody: "Uygulama, disk görüntüsü veya geçici bir sistem konumu gibi güncellenemeyen bir yerden çalışıyor. \(AppInfo.name)’i Uygulamalar klasörüne sürükleyin, oradan açın ve yeniden deneyin.",
        ...
        obContinue: "Devam",
        obBack: "Geri",
        obStart: "\(AppInfo.name)’i aç",
        obStepWelcomeTitle: "\(AppInfo.name)’e hoş geldin",
        ...
        obStepDoneBody: "\(AppInfo.name) Mac’inle ilgilenmeye başladı bile.",
        ...
        obPurposeBody: "Etkinleştirmek istediğiniz özellikleri seçin. Mevcut seçenekleriniz korunur.",
        obPurposeSkip: "Özellikleri daha sonra Ayarlar’dan değiştirebilirsiniz.",

        tabMonitor: "Monitör",
        tabMenuBarIcon: "Menü çubuğu simgesi",
        tabMenuBarPanel: "Menü çubuğu paneli",
        ...
        menuBarIconSection: "Uygulama simgesi",
        ...
        cleaningNeedsAxBody: "Klavyeyi güvenle kilitlemek için \(AppInfo.name)’in Erişilebilirlik iznine ihtiyacı var. Sistem Ayarları’ndan izin ver ve tekrar dene.",
        ...
        showMenuBarIconCaption: "\(AppInfo.name) simgesi kaybolursa (macOS, çubukta yer kalmadığında menü çubuğu simgelerini gizleyebilir; çentikli Mac’lerde yaygındır), \(AppInfo.name)’i Uygulamalar’dan veya Spotlight’tan yeniden aç: bu simgeyi yeniden oluşturur ve hâlâ gizliyse bu pencereyi açar.",
        ...
        menuBarIconManagerHintFormat: "%@ açık ve simgeyi gizli bölümünde tutuyor olabilir. \(AppInfo.name)’i orada arayın veya %@ uygulamasını \(AppInfo.name)’i her zaman gösterecek şekilde ayarlayın.",
        ...
        cleanerNotifDenied: "\(AppInfo.name) bildirimleri sistemde kapalı.",
        cleanerNotifOpenSettings: "Bildirim ayarlarını aç…",
        launchAtLoginNeedsApplications: "Uygulama, oturum açılırken açılmaya izin vermeyen bir konumdan çalışıyor. \(AppInfo.name)’i Uygulamalar klasörüne sürükleyin, oradan açın ve bu seçeneği yeniden açın.",
        launchAtLoginNeedsApproval: "Giriş ögesi kayıtlı, ancak Sistem Ayarları’nda hâlâ kapalı. Sistem Ayarları › Genel › Giriş Ögeleri ve Uzantılar’ı açın ve “Oturum açılırken aç” altında \(AppInfo.name)’i açın.",
        ...
        mouseAccelerationCaption: "Bağlı farelerde işaretçi hızlandırmasını kaldırır. Seçenek kapatıldığında veya \(AppInfo.name)’ten çıkıldığında önceki ayar geri yüklenir.",
