...
final class RadialMenuService: ObservableObject {
    ...
    /// The wheel is the only entry point left for these, so the only gate is
    /// the wheel itself.
    private func run(_ action: QuickToggleAction) {
        guard AppFeature.radialMenu.isAvailable else { return }
        // Let the non-activating wheel disappear before the action lands.
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.15) {
            let toggles = QuickTogglesService.shared
            switch action {
            case .darkMode: toggles.toggleDarkMode()
            case .emptyTrash: toggles.emptyTrashAfterConfirming()
