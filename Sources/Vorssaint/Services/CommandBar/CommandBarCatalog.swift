...
enum CommandBarCatalog {
    ...
    static func toggleEntries(_ s: Strings,
        ...
        func entry(for feature: AppFeature,
            ...
            return CommandBarEntry(
                ...
                subtitle: feature.group.title(hub),
                keywords: name,
        ...
        return AppFeature.allCases.flatMap { feature -> [CommandBarEntry] in
            ...
            if feature == .mouseButtonShortcuts {
                ...
                return [
                    entry(for: feature,
                          ...
                          name: feature.name(s, language: language),
            ...
            let name = feature.name(s, language: language)
    ...
    private static func actionEntries(_ s: Strings,
        ...
        func area(_ feature: AppFeature, under title: String = "") -> String {
            let name = feature.name(s, language: language)
            guard name == title else { return name }
            return feature.group.title(hub)
        }
        ...
        if AppFeature.screenshot.isAvailable {
            ...
            entries.append(CommandBarEntry(
                ...
                title: AppFeature.screenshot.name(s, language: language),
                subtitle: area(.screenshot, under: AppFeature.screenshot.name(s, language: language)),
        ...
        if AppFeature.screenRecorder.isAvailable {
            ...
            entries.append(CommandBarEntry(
                ...
                title: running ? recorder.stopButton : AppFeature.screenRecorder.name(s, language: language),
                subtitle: area(.screenRecorder, under: AppFeature.screenRecorder.name(s, language: language)),
        ...
        if AppFeature.screenshot.isAvailable || AppFeature.screenRecorder.isAvailable {
            ...
            entries.append(CommandBarEntry(
                ...
                title: recent.title,
                subtitle: FeatureGroup.capture.title(hub),
        ...
        if AppFeature.screenOCR.isAvailable {
            entries.append(CommandBarEntry(
                id: "action.screenOCR",
                title: AppFeature.screenOCR.name(s, language: language),
                subtitle: area(.screenOCR, under: AppFeature.screenOCR.name(s, language: language)),
        ...
        if AppFeature.colorPicker.isAvailable {
            entries.append(CommandBarEntry(
                ...
                title: AppFeature.colorPicker.name(s, language: language),
                subtitle: area(.colorPicker, under: AppFeature.colorPicker.name(s, language: language)),
        ...
        if AppFeature.scratchpad.isAvailable {
            entries.append(CommandBarEntry(
                ...
                title: AppFeature.scratchpad.name(s, language: language),
                subtitle: area(.scratchpad, under: AppFeature.scratchpad.name(s, language: language)),
        ...
        if AppFeature.shelf.isAvailable {
            ...
            entries.append(CommandBarEntry(
                id: "action.shelf",
                title: AppFeature.shelf.name(s, language: language),
                subtitle: area(.shelf, under: AppFeature.shelf.name(s, language: language)),
                icon: .symbol("tray.full"),
                shortcut: enabled ? roleShortcut(.shelf) : nil,
                trouble: enabled ? nil : .needsSetup(featureTitle: AppFeature.shelf.name(s, language: language), page: .shelf),
                run: { _ in afterBeat { ShelfService.shared.summon() } }))
        }
        if AppFeature.pastePlain.isAvailable {
            entries.append(CommandBarEntry(
                id: "action.pastePlain",
                title: AppFeature.pastePlain.name(s, language: language),
                subtitle: area(.pastePlain, under: AppFeature.pastePlain.name(s, language: language)),
        ...
        if AppFeature.cleaningMode.isAvailable {
            entries.append(CommandBarEntry(
                ...
                title: AppFeature.cleaningMode.name(s, language: language),
                subtitle: area(.cleaningMode, under: AppFeature.cleaningMode.name(s, language: language)),
        ...
        if AppFeature.brightness.isAvailable {
            ...
            // Apple only put backlight keys on some keyboards, so this is the
            // one way in for everyone else.
            if let keyboardLightOn = BrightnessService.shared.keyboardLightEnabled {
                entries.append(CommandBarEntry(
                    id: "action.keyboardLight",
                    title: FeatureStrings.brightness(language).keyboardLight,
                    subtitle: area(.brightness),
                    icon: .symbol("keyboard"),
                    isActive: keyboardLightOn,
                    run: { _ in
                        BrightnessService.shared.setKeyboardLightEnabled(!keyboardLightOn)
                    }))
            }
        }
        ...
        if AppFeature.commandBar.isAvailable {
            ...
            let togglesText = FeatureStrings.systemActions(language)
            let togglesArea = AppFeature.commandBar.name(s, language: language)
        ...
        if AppFeature.cleaner.isAvailable {
            entries.append(CommandBarEntry(
                id: "action.cleaner",
                title: AppFeature.cleaner.name(s, language: language),
                subtitle: area(.cleaner, under: AppFeature.cleaner.name(s, language: language)),
                icon: .symbol("sparkle"),
        ...
        if AppFeature.uninstaller.isAvailable {
            entries.append(CommandBarEntry(
                ...
                title: AppFeature.uninstaller.name(s, language: language),
                subtitle: area(.uninstaller, under: AppFeature.uninstaller.name(s, language: language)),
                icon: .symbol("trash"),
            ...
            if UserDefaults.standard.bool(forKey: DefaultsKey.uninstallerCommandBarEnabled) {
                entries.append(CommandBarEntry(
                    ...
                    subtitle: AppFeature.uninstaller.name(s, language: language),
                    icon: .symbol("trash"),
        ...
        entries.append(CommandBarEntry(
            ...
            subtitle: AppFeature.commandBar.name(s, language: language),
        ...
        entries.append(CommandBarEntry(
            ...
            run: { _ in openSettings(at: .features) }))
        return entries
    }
        ...
        SettingsDirectory.searchItems(s, language: language).compactMap { item in
            ...
            case .page(let page):
                guard !pagesCoveredByActions.contains(page),
                      FeatureVisibilitySupport.isPageVisible(page, isAvailable: { $0.unit.isAvailable })
                else { return nil }
                id = "settings.\(page)"
            case .feature(let feature):
                guard feature.unit.isAvailable,
                      FeatureVisibilitySupport.isPageVisible(
                        item.destination.page, isAvailable: { $0.unit.isAvailable })
                else { return nil }
    ...
    static func uninstallSelectionEntries(urls: [URL], automationDenied: Bool) -> [CommandBarEntry] {
        ...
        return [CommandBarEntry(
            ...
            subtitle: AppFeature.uninstaller.name(L10n.shared.s, language: L10n.shared.language),
