...
final class CommandBarService: ObservableObject {
    ...
    /// What the Homebrew confirmation covers: the package, app and checked
    /// items at the moment it was asked, so a changed review cannot be
    /// confirmed by an older answer.
    ...
    var returnActionTitle: String? {
        ...
        case .uninstallReview:
            // Return in the field does what the checklist's primary button does.
            let uninstaller = AppUninstaller.shared
            switch uninstaller.phase {
            case .results:
                return uninstaller.selectedHomebrewPackage == nil
                    ? L10n.shared.s.uninstallerRemove : L10n.shared.s.homebrewUninstall
            case .done: return L10n.shared.s.uninstallerAnother
            case .empty, .scanning, .removing: return nil
            }
        case .uninstallHomebrewConfirm:
            return L10n.shared.s.homebrewUninstall
    ...
    private func confirmUninstallReview(entryID: String) {
        let uninstaller = AppUninstaller.shared
        guard uninstaller.phase == .results, !uninstaller.isBusy else { return }
    ...
    private func beginUninstallReview(appURL url: URL, entryID: String) {
        ...
        guard AppFeature.uninstaller.isAvailable,
            ...
            uninstallWarning = uninstaller.isBusy
    ...
    private func run(_ entry: CommandBarEntry) {
        invalidateDestinationAttempt()
        uninstallFinderRequestID = nil
        let decision = runDecision(for: entry, query: query)
        if decision != .setup, let url = entry.uninstallAppURL {
