...
/// camera image, summoned from the menu panel or a global shortcut. The
/// capture session exists only while the panel is on screen, so the camera
/// light and every resource die the moment it closes.
final class CameraPreviewService: ObservableObject {
        ...
        case .notDetermined:
            ...
            AVCaptureDevice.requestAccess(for: .video) { [weak self] granted in
                DispatchQueue.main.async {
                    ...
                    guard let self, self.isVisible, self.captureGeneration == generation else { return }
    ...
    func retryCapture() {
        guard isVisible, state == .unavailable else { return }
        beginCapture()
    }
    ...
    private func ensurePanel() -> NSPanel {
        ...
        let panel = KeyablePreviewPanel(contentRect: NSRect(x: 0, y: 0, width: 320, height: 240),
                                        ...
                                        backing: .buffered,
                                        defer: false)
        panel.title = FeatureStrings.cameraPreview(L10n.shared.language).pageTitle
