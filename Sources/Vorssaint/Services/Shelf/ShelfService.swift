...
final class ShelfService: ObservableObject {
    ...
    private final class KeyableShelfPanel: NSPanel, NSDraggingDestination {
        func draggingEntered(_ sender: NSDraggingInfo) -> NSDragOperation {
            ...
            let badgeOnly = self === shelf.dockedPanel && shelf.dockedPlacement == .topCenter && !shelf.dockedExpanded
            let accepts = !badgeOnly && !shelf.isInternalDragActive && shelf.canAcceptPasteboard(sender.draggingPasteboard)
    ...
    func acceptDrop(pasteboard: NSPasteboard) -> Bool {
        ...
        let companions = receivers.isEmpty ? (items: [], positions: [], promises: []) : nonPromisedItems(from: pasteboard)
        return receivers.isEmpty
            ? accept(pasteboard: pasteboard)
            : beginPromisedFileReceive(receivers, additions: companions.items, companionPositions: companions.positions, promisePositions: companions.promises, mergeInto: nil)
    }
    ...
    func merge(draggingInfo: NSDraggingInfo, into targetID: UUID) -> Bool {
        ...
        let accepted = receivers.isEmpty
            ...
            : beginPromisedFileReceive(receivers, additions: companions.items, companionPositions: companions.positions, promisePositions: companions.promises, mergeInto: targetID)
        if accepted, draggingInfo.draggingDestinationWindow === dockedPanel { dockDidAccept() }
        return accepted
    }
    ...
    private func beginPromisedFileReceive(_ receivers: [NSFilePromiseReceiver], additions: [Item],
                                          companionPositions: [Int], promisePositions: [Int],
        ...
        let store = storeDirectory ?? tempDir
        let id = UUID()
        guard let transfer = ShelfFilePromiseTransfer(temporaryDirectory: tempDir,
            ...
            let receivedItems = ShelfPasteboardSupport.mergedItemIndices(
                companionPositions: companionPositions, receiverIndices: result.receiverIndices,
                promisePositions: promisePositions).map { combined[$0] }
            let added: Bool
    ...
    private func reportPromiseDeliveryProblem(title: String, body: String) {
        ...
        if let window = panel, window.isVisible {
            ...
            QuickToolHUD.show(icon: "exclamationmark.triangle", message: title + "\n" + body)
        }
    }
    ...
    private func nonPromisedItems(from pasteboard: NSPasteboard) -> (items: [Item], positions: [Int], promises: [Int]) {
        ...
        for (index, entry) in (pasteboard.pasteboardItems ?? []).enumerated() {
            if entry.types.contains(where: { ShelfPasteboardSupport.isFilePromiseType($0.rawValue) }) {
                ...
                let companions = items(from: entry)
                result.items += companions
                result.positions += Array(repeating: index, count: companions.count)
            }
        }
        return result
    }
