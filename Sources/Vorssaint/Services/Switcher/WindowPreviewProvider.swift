...
final class WindowPreviewProvider {
    ...
    /// window's capture, so each backing window is captured once. Capture
    /// pauses while an app listed under `excludedAppsKey` is in front.
    ...
    private func scheduleWarm(pid: pid_t) {
        // Warming is the switcher's: only AppSwitcher starts it.
